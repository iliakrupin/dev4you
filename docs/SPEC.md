...
2. **«Откати последнее изменение темы»** — агент восстанавливает прежнее содержимое файлов из родительского коммита (без локального git — читает blob'ы через Octokit и коммитит поверх).
...
Двухуровневая защита: что можно **редактировать** и что нельзя **удалять**. Источник истины —
[`lib/agent/sandbox.ts`](../lib/agent/sandbox.ts); ниже — резюме.

### Whitelist для редактирования (`isAllowed`)

- `app/globals.css` — CSS-переменные темы
- `tailwind.config.{ts,js,mjs,cjs}` — конфиг Tailwind (если есть)
- `app/page.tsx`, `app/new/page.tsx`, `app/new/new-task-form.tsx`,
  `app/tasks/[id]/page.tsx`, `app/tasks/[id]/task-detail.tsx`, `app/(public)/**` — публичные страницы UI
- `components/**` (кроме `auto-refresh` / `list-auto-refresh` — клиентская инфраструктура)
- `public/**` — статика (svg, png, jpg, jpeg, webp, ico, json, txt)
...
| API | Next.js Route Handlers. Пайплайн — **Edge runtime** (25s default, увеличено через `maxDuration`); webhooks/reset/cron — nodejs (источник истины — `export const runtime` в каждом route) |
...
- Local Qwen-сервер по адресу без домена (адрес не публикуется).
- **Решение:** прозрачная подмена в коде на `<адрес>.nip.io` (publicly hosted wildcard DNS). Обходит ограничение, при использовании OpenRouter не активируется.
